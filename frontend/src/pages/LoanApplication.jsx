...
export default function LoanApplication() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (activeLoanInfo) {
      setError("Active loan restriction: You must clear your existing active loan before applying for a new one.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white flex flex-col font-sans">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 w-full">
        ...
        {activeLoanInfo && (
          <div className="mb-6 p-6 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-300 space-y-4">
            <div className="flex items-center gap-3">
              <AlertCircle className="w-6 h-6 text-amber-400 shrink-0" />
              <div>
                <h3 className="font-extrabold text-base text-amber-200">Active Loan Restriction Policy</h3>
                <p className="text-xs text-amber-300/90 mt-0.5">
                  You currently have an active loan file <strong>#{activeLoanInfo?.loan_details?.application_id}</strong> (Status: <strong>{activeLoanInfo?.loan_status}</strong>).
                  Each customer can hold only <strong>one active loan at a time</strong>. You must clear/repay your active loan before submitting a new application.
                </p>
              </div>
            </div>
            <div className="pt-2 flex justify-end">
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-amber-500 text-slate-950 font-extrabold text-xs hover:bg-amber-400 transition-all flex items-center gap-2"
              >
                Go to Dashboard to Repay & Clear Loan <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        )}
        ...
        {isSubmitting ? (
          ...
          <form onSubmit={handleSubmit} className="p-8 rounded-2xl bg-[#161f32]/80 border border-[#23304a] space-y-6">
            ...
            {/* Low CIBIL & First-Time Borrower Info Banner */}
            <div className="p-4 rounded-xl bg-gradient-to-r from-amber-500/10 via-indigo-500/10 to-emerald-500/10 border border-amber-500/30 text-xs text-slate-200 flex items-start gap-3">
              <Sparkles className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
              <div>
                <strong className="text-amber-300 font-bold block">First-Time Borrower or Low CIBIL Score?</strong>
                <span className="text-slate-300 text-[11px] leading-relaxed">
                  If your CIBIL score is low or you have no credit history, our AI Multi-Agent pipeline uses your <strong className="text-white">Alternative Data Consents</strong> (Utility bills, Bank cashflow, LinkedIn) to award up to <strong className="text-emerald-400">+160 score points</strong> compensation and approve your loan!
                </span>
              </div>
            </div>
